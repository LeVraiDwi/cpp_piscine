...
void				Form::execute(Bureaucrat const & bcrat) const{
	if (bcrat.getGrade() > this->_executable)
...
Form::FormNotSigned::FormNotSigned(){
	std::cout << "construction FormNotSigned" << std::endl;
	return;
}

Form::FormNotSigned::~FormNotSigned(){
	std::cout << "destructeur FormNotSigned" << std::endl;
	return;
}
